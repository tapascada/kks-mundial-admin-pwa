...
function resolveKnockouts() {
  ...
  const getWinner = (matchId, fallback) => {
    const m = matchMap.get(matchId);
    if (!m) return fallback;
    const g1 = m.realGoals1;
    const g2 = m.realGoals2;
    const isCompleted = m.status === 'TERMINADO' || (g1 !== null && g1 !== undefined && g2 !== null && g2 !== undefined);
    if (isCompleted && g1 !== null && g2 !== null) {
  ...
  const getLoser = (matchId, fallback) => {
    const m = matchMap.get(matchId);
    if (!m) return fallback;
    const g1 = m.realGoals1;
    const g2 = m.realGoals2;
    ...
    if (isCompleted && g1 !== null && g2 !== null) {
      ...
      if (m.penaltiesWinner) {
        return m.penaltiesWinner.toLowerCase() === m.team1.toLowerCase() ? m.team2 : m.team1;
      }
    }
    return fallback;
  };
  ...
  octavosPairings.forEach(p => {
    const m = matchMap.get(p.id);
    if (m && (m.status === 'PENDIENTE' || m.status === 'PREVIA' || !m.status)) {
      m.team1 = getWinner(p.pm1, `Ganador partido ${p.pm1}`);
      m.team2 = getWinner(p.pm2, `Ganador partido ${p.pm2}`);
    }
  });
  ...
  const cuartosPairings = [
    ...
    { id: 98, pm1: 93, pm2: 94 },
    { id: 99, pm1: 91, pm2: 92 },
    { id: 100, pm1: 95, pm2: 96 }
  ];

  cuartosPairings.forEach(p => {
    const m = matchMap.get(p.id);
    if (m && (m.status === 'PENDIENTE' || m.status === 'PREVIA' || !m.status)) {
      m.team1 = getWinner(p.pm1, `Ganador partido ${p.pm1}`);
      m.team2 = getWinner(p.pm2, `Ganador partido ${p.pm2}`);
  ...
  // Semis (101 & 102)
  const semisPairings = [
    { id: 101, pm1: 97, pm2: 98 },
    { id: 102, pm1: 99, pm2: 100 }
  ];

  semisPairings.forEach(p => {
    const m = matchMap.get(p.id);
    if (m && (m.status === 'PENDIENTE' || m.status === 'PREVIA' || !m.status)) {
      m.team1 = getWinner(p.pm1, `Ganador partido ${p.pm1}`);
      m.team2 = getWinner(p.pm2, `Ganador partido ${p.pm2}`);
  ...
  // Tercer Puesto (103)
  const tercerPuesto = matchMap.get(103);
  if (tercerPuesto && (tercerPuesto.status === 'PENDIENTE' || tercerPuesto.status === 'PREVIA' || !tercerPuesto.status)) {
    tercerPuesto.team1 = getLoser(101, "Perdedor partido 101");
    tercerPuesto.team2 = getLoser(102, "Perdedor partido 102");
  }

  // Final (104)
  const granFinal = matchMap.get(104);
  if (granFinal && (granFinal.status === 'PENDIENTE' || granFinal.status === 'PREVIA' || !granFinal.status)) {
    granFinal.team1 = getWinner(101, "Ganador partido 101");
    granFinal.team2 = getWinner(102, "Ganador partido 102");
  }
}
